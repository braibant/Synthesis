...
Variable type : Type. 
Variable Bool : type. 
Variable expr : list type -> type -> Type. 

Section shallow.   

  Fixpoint f ( l : list Type) : Type := 
    match l with 
      | nil => unit
      | cons t nil => t
      | cons t q => (t * f q)
  ...
  Inductive machine (sigma : Type):=
    {
      children : list Type;
      instances : dlist machine children; 
      reset : sigma;
      step : sigma * f children -> sigma * f children
    }. 
...
Section expr.   
  ...
  Inductive com : Type :=
    | Cskip : com 
    | Cseq : com -> com -> com 
    | Cset_mem : forall t, var  env t -> expr env t -> com
    | Cifb : expr env Bool -> com -> com -> com
    | Cstep : forall i, var  instance i ->  com. 
...
Inductive machine :=
  {
    ...
    mems : list type;
    step : com mems machine instances                     
  }. 
  ...
  Inductive com memsinstances_eval : com memsinstances -> state -> state -> Prop :=
  ...
  | Eifb_false : forall (e : expr Tbool) c1 c2 st1 st2, 
                   ...
                   c2 / st1 ==> st2 ->
                   (Cifb e c1 c2) / st1 ==> st2 
   where "c1 '/' st '==>' st'" := (com memsinstances_eval c1 st st').

Fixpoint com memsinstances_denote (c : com memsinstances) (st1 : state) : option state :=
  match c with
    | Cskip => Some st1
    | Cseq c1 c2 => 
        do st2 <- com memsinstances_denote c1 st1;
        com memsinstances_denote c2 st2
    | Cset_reg t v e =>
    ...
    | Cifb e c1 c2  =>
        let r := eval_expr st1 e in 
          if r then com memsinstances_denote c1 st1 
          else  com memsinstances_denote c2 st1
  end. 
End expr.  
...
Record RTL :=
  {
    mems : list type0;
    regs : list type0;
    code : com memsinstances mems regs; 
...
Section semantics. 
  ...
  (* intros. apply com memsinstances_denote. apply (code  R).  *)
  (* apply X.  *)
  (* Defined.  *)
...
Inductive machine :=
  {
    ...
    regs : list type0;
    instances : list machine;
    reset : com memsinstances mems regs ;
    step : com memsinstances mems regs
  }. 
...
Inductive com memsinstances_eval m : com memsinstances (mems m) (regs m) -> state m -> state m -> Prop :=
  ...
  | Eset_mem : forall t (v : var mems t) (e : expr t) st1 st2 r, 
    ...
    | Eifb_false : forall (e : expr Tbool) c1 c2 st1 st2, 
               ...
               c2 / st1 ==> st2 ->
               (Cifb e c1 c2) / st1 ==> st2 *)
    where "c1 '/' st '==>' st'" := (com memsinstances_eval _ c1 st st').
